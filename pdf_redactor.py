...
def update_text_layer(options, text_tokens, page_tokens):
	...
	tagged_text = util.preprocess_text(text_content)
	parsers=[CityParser(), CountryParser(), NameParser(), IPAddressParser(), RoutingNumberParser(),SSNParser(), EmailParser(), USAPhoneParser(), GenderParser(), EINParser(), CCNumberParser(),CompanyParser()]
	pii=[]
	for parser in parsers:
		pii.extend(parser.detect_pii(text_content,tagged_text))
	text_tokens_index = 0
